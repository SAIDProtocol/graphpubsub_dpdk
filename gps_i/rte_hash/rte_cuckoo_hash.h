...
#ifndef _RTE_CUCKOO_HASH_X_H_
#define _RTE_CUCKOO_HASH_X_H_
 ...
 */
const rte_hash_cmp_eq_t cmp_jump_table_x[NUM_KEY_CMP_CASES] = {
	NULL,
	rte_hash_k16_cmp_eq,
	rte_hash_k32_cmp_eq,
	rte_hash_k48_cmp_eq,
...
struct lcore_cache_x {
	unsigned len; /**< Cache len */
	void *objs[LCORE_CACHE_SIZE]; /**< Cache objects */
} __rte_cache_aligned;

/* Structure that stores key-value pair */
struct rte_hash_key_x {
	union {
		uintptr_t idata;
		void *pdata;
	};
...
enum rte_hash_sig_compare_function_x {
...
struct rte_hash_x {
	...
	struct lcore_cache_x *local_free_slots;
	...
	enum cmp_jump_table_case_x cmp_jump_table_idx;
	...
	struct rte_hash_bucket_x *buckets;
	/**< Table with buckets storing all the	hash values and key indexes
	 * to the key table.
	 */
	rte_rwlock_t *readwrite_lock; /**< Read-write lock thread-safety. */
	struct rte_hash_bucket_x *buckets_ext; /**< Extra buckets array */
...
struct queue_node_x {
	struct rte_hash_bucket_x *bkt; /* Current bucket on the bfs search */
	uint32_t cur_bkt_idx;

	struct queue_node_x *prev;     /* Parent(bucket) in search path */
